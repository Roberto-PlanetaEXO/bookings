...
<html <?php language_attributes(); ?>>
	...
	if ( isset( $email ) && is_a( $email, 'WC_Email_Customer_Completed_Order' ) ) {
		$order = $email->object;
...
<div style="margin-bottom: 40px;">
		<table class="td" cellspacing="0" cellpadding="0" style="width: 100%; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; background: #fff" border="0">
			<?php
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML do template de e-mail
			echo function_exists( 'pxo_processing_custom_email_order_items_table' )
				? pxo_processing_custom_email_order_items_table( $order, array( 'show_sku' => $sent_to_admin, 'show_image' => false, 'image_size' => array( 32, 32 ), 'plain_text' => $plain_text, 'sent_to_admin' => $sent_to_admin ) )
				: wc_get_template_html( 'emails/email-order-items.php', array( 'order' => $order, 'items' => $order->get_items(), 'show_sku' => $sent_to_admin, 'show_image' => false, 'image_size' => array( 32, 32 ), 'plain_text' => $plain_text, 'sent_to_admin' => $sent_to_admin ) );
			?>
		</table>
